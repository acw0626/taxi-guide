...
public class TaximanagePolicyHandler {
	...
	@StreamListener(KafkaProcessor.INPUT)
	public void wheneverTaximanageAssigned_(@Payload TaximanageAssigned 택시할당요청됨) {
		...
		if (택시할당요청됨.isMe()) {
			...
			if (택시할당요청됨.getId() != null)
				// Correlation id 는 '고객휴대폰번호' 임
				txMgrRepository.findById(Long.valueOf(택시할당요청됨.getId())).ifPresent((Taximanage) -> {
					Taximanage.setStatus(택시할당요청됨.getStatus());
					txMgrRepository.save(Taximanage);
				});
